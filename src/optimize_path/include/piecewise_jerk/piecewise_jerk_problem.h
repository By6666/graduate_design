...
class PiecewiseJerkProblem {
 public:
  ...
  void set_x_ref(std::vector<double> x_ref);

  void set_end_state_ref(const std::array<double, 3>& weight_end_state,
                         const std::array<double, 3>& end_state_ref);

  inline DifferentialLayer& set_layers() { return layers_; }

  inline const DifferentialLayer& get_layers() const { return layers_; }
