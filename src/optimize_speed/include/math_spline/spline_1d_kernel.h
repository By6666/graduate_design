...
#ifndef MATH_SPLINE_SPLINE_1D_KERNEL_H_
#define MATH_SPLINE_SPLINE_1D_KERNEL_H_
...
class Spline1dKernel {
 public:
  ...
  bool AddReferenceKernelToMainKernel();
  ...
  bool AddReferenceLineKernelMatrix(const std::vector<double>& x_coord,
                                    const std::vector<double>& ref_fx, const double weight);
  bool AddReferenceSpeedKernelMatrix(const std::vector<double>& x_coord,
                                     const std::vector<double>& ref_dx, const double weight);
 ...
 private:
  ...
  void AddNthDerivativekernelMatrixForSplineK(const uint32_t n, const uint32_t k,
                                              const double weight);
 ...
 private:
  ...
  Eigen::MatrixXd ref_kernel_matrix_;
  Eigen::MatrixXd ref_offset_;
};

}  // namespace qp_spline
}  // namespace math

#endif  // MATH_SPLINE_SPLINE_1D_KERNEL_H_
