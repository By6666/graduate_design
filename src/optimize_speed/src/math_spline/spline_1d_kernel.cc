...
#include "math_spline/spline_1d_kernel.h"
#include "common/log.h"

#include <algorithm>

#include "math_spline/spline_seg_kernel.h"
...
Spline1dKernel::Spline1dKernel(const std::vector<double>& x_knots, const uint32_t spline_order)
  ...
  ref_kernel_matrix_ = Eigen::MatrixXd::Zero(total_params_, total_params_);
  ref_offset_ = Eigen::MatrixXd::Zero(total_params_, 1);
}
...
bool Spline1dKernel::AddKernel(const Eigen::MatrixXd& kernel, const Eigen::MatrixXd& offset,
                               const double weight) {
...
void Spline1dKernel::AddNthDerivativekernelMatrixForSplineK(const uint32_t n, const uint32_t k,
                                                            const double weight) {
...
bool Spline1dKernel::AddReferenceLineKernelMatrix(const std::vector<double>& x_coord,
  ...
  if (std::fabs(weight) < __DBL_EPSILON__) {
    return true;
  }
  ref_kernel_matrix_.setZero();
  ref_offset_.setZero();
  ...
  for (uint32_t i = 0; i < x_coord.size(); ++i) {
    ...
    for (uint32_t j = 0; j < num_params; ++j) {
      ref_offset_(j + cur_index * num_params, 0) += offset_coef;
    ...
    ref_kernel_matrix_.block(cur_index * num_params, cur_index * num_params, num_params,
                             num_params) += weight * ref_kernel;
  }
  return true;
}

bool Spline1dKernel::AddReferenceSpeedKernelMatrix(const std::vector<double>& x_coord,
                                                   const std::vector<double>& ref_dx,
                                                   const double weight) {
  if (ref_dx.size() != x_coord.size()) {
    return false;
  }
  if (weight < __DBL_EPSILON__) {
    return true;
  }
  const uint32_t num_params = spline_order_ + 1;
  for (uint32_t i = 0; i < x_coord.size(); ++i) {
    ...
    double cur_rel_x = x_coord[i] - x_knots_[cur_index];
    // update offset
    double offset_coef = -2.0 * ref_dx[i] * weight;
    for (uint32_t j = 1; j < num_params; ++j) {
      ref_offset_(j + cur_index * num_params, 0) += j * offset_coef;
      offset_coef *= cur_rel_x;
    }
    ...
    for (uint32_t r = 0; r < num_params; ++r) {
      for (uint32_t c = 0; c < num_params; ++c) {
        ref_kernel(r, c) = 2.0 * power_x[r + c] * r * c;
      }
    }
...
bool Spline1dKernel::AddReferenceKernelToMainKernel() {
  return AddKernel(ref_kernel_matrix_, ref_offset_, 1.0);
}
