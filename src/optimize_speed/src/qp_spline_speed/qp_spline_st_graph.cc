...
void QpSplineStGraph::Init() {
  ...
  spline_generator_ =
      std::make_shared<Spline1dGenerator>(t_knots_, FLAGS_spline_order);
}

void QpSplineStGraph::SetCondition(const CommonPoint& init_point,
                                   const Limits& s_limits,
                                   const Limits& v_limits,
                                   const std::pair<double, double>& a_limits,
                                   const References& s_refs,
...
bool QpSplineStGraph::Solve(SpeedData* const speed_data) {
  ...
  AERROR << "Solve qp problem successful !!";
  ...
  while (time < FLAGS_total_time) {
    ...
    speed_data->AppendSpeedPoint(time, s, v, a, da);
...
bool QpSplineStGraph::AddConstraint() {
  ...
  CommonGetConstraintForSolve(v_limits_, &speed_lower_bound,
                              &speed_upper_bound);
...
void QpSplineStGraph::CommonGetConstraintForSolve(
    const Limits& limits, std::vector<double>* const lower_bounds,
  ...
  for (const double curr_t : t_evaluated_) {
    ...
    ADEBUG << "Add constraint by time: [t, l, u] => [" << curr_t << ", "
...
void QpSplineStGraph::CommonGetReferenceForSolve(
    const References& refs, std::vector<double>* const x_coord,
  ...
  for (const auto& ref : s_refs_.reference_points()) {
    ...
    ADEBUG << "Add reference by time: [t, ref] => [" << ref.x() << ", "
           << ref.ref() << "]";
  }
...
bool QpSplineStGraph::AddKernel() {
  ...
  (*spline_kernel->mutable_kernel_matrix())(2, 2) +=
      2.0 * 4.0 * FLAGS_init_acc_target_weight;
...
bool QpSplineStGraph::AddCruiseReferenceLineKernel(const double weight) {
  ...
  spline_kernel->AddReferenceLineKernelMatrix(
      t_coord, s_refs, weight * FLAGS_total_time / t_evaluated_.size());
...
bool QpSplineStGraph::AddSpeedRefKernel(const double weight) {
  ...
  //   speed_ref_with_time.emplace_back(SpeedData(EstimateSpeedRefByInitSpeed(init_point,
  //   speed)));
  // }
...
//                                                 weight * FLAGS_total_time /
//                                                 t_pos.size());
//   }
//   return true;
// }

// bool QpSplineStGraph::GetSConstraintByTime(const double time double* const
// s_upper_bound,
...
//   last_frame->DriveReferenceLineInfo(); if (!last_reference_line_info) {
...
//     const common::TrajectoryPoint& init_point, const SpeedData& speed_ref)
//     const {
//   std::vector<common::SpeedPoint> speed_ref_with_time =
//   speed_ref.speed_vector(); for (size_t i = 0; i <
//   speed_ref_with_time.size(); i++) {
//     speed_ref_with_time.at(i).set_t(speed_ref_with_time.at(i).s() /
//     init_point.v());
//   }
//   return speed_ref_with_time;
// }

// void QpSplineStGraph::LinearFillingInsideSpeedRef(const
// std::vector<SpeedData>& speed_ref,
//                                                   std::vector<double>* time,
...
// bool QpSplineStGraph::EstimateSpeedUpperBound(const common::TrajectoryPoint&
// init_point,
//                                               const SpeedLimit& speed_limit,
//                                               std::vector<double>*
//                                               speed_upper_bound) const {
...
//   speed_upper_bound->push_back(std::max(v,
//   speed_limit.speed_limit_points().front().second));
...
//     uint32_t i = 1;
//     uint32_t j = 0;
//     while (i < t_evaluated_.size() && j + 1 <
//     speed_limit.speed_limit_points().size()) {
//       double distance = v * t_evaluated_[i];
//       if (!last_speed_data.Empty() && distance <
//       last_speed_data.speed_vector().back().s()) {
//         SpeedPoint p;
...
//     for (uint32_t k = speed_upper_bound->size(); k < t_evaluated_.size();
//     ++k) {
...
//       const auto& it =
//           std::lower_bound(speed_limit_points.begin(),
...
//       speed_upper_bound->at(k) *= (1.0 +
//       FLAGS_change_lane_speed_relax_percentage);
//     }
//   }

//   const double kTimeBuffer = 1.0;
//   const double kSpeedBuffer = 0.1;
//   for (uint32_t k = 0; k < t_evaluated_.size() && t_evaluated_[k] <
//   kTimeBuffer; ++k) {
